...
class DataFetcherManager:
    ...
    def get_daily_data(
        ...
        # --- 兼容美股/港股/指数等非 A 股代码：优先尝试 yfinance ---
        # 规则（尽量宽松，不影响 A 股）：
        # - 含字母（AAPL/TSLA/BRK-B/BTC-USD 等）
        # - 以 ^ 开头（指数，如 ^GSPC）
        # - 含 '-'（美股特殊代码，如 BRK-B）
        # - 包含 '.' 且不是 A 股常见后缀（.SS/.SZ/.SH）
        #
        # 这样可以避免 A 股数据源先“轮流失败”造成日志噪声，也更快。
        ucode = (stock_code or '').strip().upper()
        looks_non_cn = (
            any(ch.isalpha() for ch in ucode)
            or ucode.startswith('^')
            or '-' in ucode
            or ('.' in ucode and not ucode.endswith(('.SS', '.SZ', '.SH')))
        )
        ...
        if looks_non_cn:
            fetchers_to_try = [
                ...
                if getattr(f, 'name', '') == 'YfinanceFetcher'
            ] + [
                f for f in self._fetchers
                if getattr(f, 'name', '') != 'YfinanceFetcher'
            ]
