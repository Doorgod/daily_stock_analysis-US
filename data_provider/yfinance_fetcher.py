...
class YfinanceFetcher(BaseFetcher):
    ...
    def _convert_stock_code(self, stock_code: str) -> str:
        ...
        """
        code_raw = (stock_code or '').strip()
        code = code_raw.upper()

        # 1) 已有后缀：A股/港股/其它交易所/指数/加密 等，尽量不动
        # - .SS/.SZ/.SH/.HK 直接返回
        # - 其它带点的（如 .NS/.L/.TO 等）也直接返回
        if '.' in code:
            return code

        # 2) 指数或含字母：美股/ETF/加密等（AAPL, TSLA, BRK-B, BTC-USD, ^GSPC）
        if code.startswith('^') or any(c.isalpha() for c in code) or ('-' in code):
            return code

        # 3) 去除可能的 A 股后缀（兼容用户输入 600519.SH / 000001.SZ 等）
        #    这里在 “不带点” 的分支里，基本用不到，但保留无害。
        ...
        if code.isdigit():
            ...
            if len(code) == 6:
                ...
                if code.startswith(('000', '001', '002', '003', '300')):
                    return f"{code}.SZ"
                # 未识别的 6 位数字，默认深市（保持原逻辑）
                logger.warning(f"无法确定股票 {code} 的市场，默认使用深市")
                return f"{code}.SZ"

            # 港股常见：0700 / 9988（4~5位数字）
            if len(code) in (4, 5):
